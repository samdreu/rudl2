...
|---|---|---|---|---|
| 2 | The synthesizable **register set is inferred from control flow** (not read off rustc's over-capturing `Future` layout) — *and it is correct* | **Structural reg-match vs independent hand-written SV** (G2), **wired into `tests/common::EquivalenceTest`** for 4 independent references: `mac_fsm` (name-exact), `det_010`/`det_110101`/`lfsr` (storage-equivalent). **Reconciled with codegen** — `copper-codegen/tests/register_reconciliation.rs` proves, corpus-wide (17 sequential modules), that codegen's emitted flip-flops equal the shared inference (+ only the synthetic phase/pc counter). So the shared analysis is now the authoritative *spec* for codegen's register set. | C1 | **DONE for `sequential` modules; NOT for `synchronizer` modules** — both proof artifacts are sequential-scoped (`register_reconciliation.rs` filters on `#[hardware(sequential)]`), and lifting that filter shows inference under-reports the 2-FF synchronizer's second flop (1 vs 2; see `tests/cdc_synchronizer_anchor.rs` and TODO P3, 2026-08-21). Consequently codegen *consuming* the set directly is **not** the behavior-neutral follow-up it was recorded as — it would drop a flop on every synchronizer until the rule is extended |
