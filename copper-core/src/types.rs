...
impl<Domain: ClockDomain, T> Signal<Domain, T> {
    ...
    /// Write a new value to the signal (committed by executor)
    pub fn write(&self, value: T) {
        self.inner.lock().unwrap().next = Some(value);
    }
    
    /// Internal: advance this signal (called by executor)
    pub(crate) fn advance_internal(&self) where T: Clone {
    ...
    /// Commit the pending signal value (useful for driving inputs)
    pub fn commit(&self) where T: Clone {
...
impl<Domain: ClockDomain, T: fmt::Debug> fmt::Debug for Signal<Domain, T> {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        ...
        self.inner.lock().unwrap().current.fmt(f)?;
        write!(f, ")")
    }
}
...
// This struct holds the actual current/next values.
// It's wrapped in Arc<Mutex<...>> for shared ownership.
...
/// Wrapper for sequential state in hardware modules
/// 
/// `State<T>` represents a hardware register that:
/// - Holds a current value (readable immediately)
/// - Buffers a next value (committed on clock edge)
/// - Uses shared ownership so the executor can commit it
/// 
/// # Why Arc<Mutex<...>>?
/// - **Arc**: Multiple references (module + executor both hold it)
/// - **Mutex**: Interior mutability (set_next modifies through &self)
/// 
/// # Example
/// ```ignore
/// let mut count = factory.create(0u32);  // Create + auto-register
/// count.set_next(count.current() + 1);   // Schedule next value
/// // Executor commits it after polling all tasks
/// ```
#[derive(Clone)]
pub struct State<T> {
...
impl<T> State<T> {
    /// Create new state with an initial value
    /// 
    /// **Note:** This does NOT auto-register. Use `StateFactory::create()` instead.
    pub fn new(initial: T) -> Self {
        Self {
    ...
    /// Read the current state value (what was set last cycle)
    pub fn current(&self) -> T where T: Clone {
        self.inner.lock().unwrap().current.clone()
    }
    
    /// Alias for current() - kept for backwards compat
    pub fn current_clone(&self) -> T where T: Clone {
        self.current()
    }
    
    /// Set the next state value (takes effect after clock edge)
    /// 
    /// This buffers the value. The executor commits it atomically
    /// with all other states after polling tasks.
    pub fn set_next(&mut self, value: T) {
        self.inner.lock().unwrap().next = Some(value);
    }
    ...
    /// Internal: Get Arc for executor registration
    /// 
    /// This is used by StateFactory to register the state.
    /// The executor holds this Arc and calls advance_internal().
    pub fn as_arc(&self) -> Arc<Mutex<StateInner<T>>> where T: Clone + 'static {
        Arc::clone(&self.inner)
    }
    
    /// Internal: Advance this state (called by executor)
    /// 
    /// Moves `next` to `current`. This happens atomically for all
    /// states after all tasks are polled.
    pub fn advance_internal(&self) where T: Clone {
        let mut inner = self.inner.lock().unwrap();
        if let Some(next) = inner.next.take() {
            inner.current = next;
        }
    ...
    /// Advance this state manually (for tests or standalone use)
    pub fn advance(&self) where T: Clone {
        self.advance_internal();
    }
}
...
impl<T: fmt::Debug + Clone> fmt::Debug for State<T> {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
...
// Remove these completely (they're replaced by the executor approach):
// - trait StateCell
// - impl<T> StateCell for State<T>
// - static STATE_REGISTRY
// - fn state_registry()
// - State::register()
// - State::commit_all()
...
mod tests {
    ...
    fn test_signal_same_domain() {
        let sig_a: Signal<TestClkA, Bits<8>> = Signal::new(Bits::from_u128(42));
