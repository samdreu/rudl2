...
pub struct Memory<T, const R: usize, const W: usize, D: ClockDomain> {
    inner: RefCell<MemoryInner<T, R, W>>, // Actual memory state
    clock: Clock<D>, // Clock
    write_mode: WriteMode, // Write-mode
    read_mode: ReadMode, // Read-mode
}
...
impl<T: Clone + Default, const R: usize, const W: usize, D: ClockDomain> Memory<T, R, W, D> {
    ...
    pub fn new(clock: Clock<D>, size: usize) -> Self {
        ...
        Memory {
            inner: RefCell::new(Self::make_inner(vec![T::default(); size], cycle)),
            clock,
    ...
    pub fn from_contents(clock: Clock<D>, data: Vec<T>) -> Self {
        let cycle = clock.cycle();
        Memory {
            inner: RefCell::new(Self::make_inner(data, cycle)),
            clock,
            write_mode: WriteMode::ReadFirst,
            read_mode: ReadMode::Sync,
        }
    ...
    ReadPort<'a, T, R, W, D, I>
{
    ...
    pub fn read(&self, addr: usize) -> T {
        ...
        // In write-first mode, check if there's a staged write to this address
...
// TODO: Add `Index` impl for `ReadPort` so `rp[addr]` works like `read(addr)` — also sets the read address for output capture.
    ...
    WritePort<'a, T, R, W, D, I>
{
    ...
    pub fn write(&self, addr: usize, value: T) {
        let inner = &mut self.mem.inner.borrow_mut();
        debug_assert!(
            !inner.write_used[I],
